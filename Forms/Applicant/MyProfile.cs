        ...
        private void LoadProfile()
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    // Load registration data from ApplicantAccounts
                    string accountQuery = "SELECT Email, FirstName, Surname, Sex, ContactInfo FROM ApplicantAccounts WHERE ApplicantAccountID = @id"; MySqlCommand accountCmd = new MySqlCommand(accountQuery, conn);
                    ...
                    using (var accountReader = accountCmd.ExecuteReader())
                    {
                        if (accountReader.Read())
                        {
                            txtEmail.Text = accountReader["Email"]?.ToString() ?? "";
                            txtFirstName.Text = accountReader["FirstName"]?.ToString() ?? "";
                            txtLastName.Text = accountReader["LastName"]?.ToString() ?? "";

                            string gender = accountReader["Gender"]?.ToString() ?? "";
                            if (cmbGender.Items.Contains(gender))
                                cmbGender.SelectedItem = gender;

                            txtPhone.Text = accountReader["Phone"]?.ToString() ?? "";
                        }
                    }

                    // Load the rest from Applicants table
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    // Check if a record already exists for this applicant
